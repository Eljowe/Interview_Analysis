...
def extract_names_and_roles(data, interviewee_speaker):
    ...
    first_interviewee_segments = [segment for segment in data if segment["speaker"] == interviewee_speaker][:10]
    first_interviewer_segments = [segment for segment in data if segment["speaker"] == interviewer_speaker][:10]
...
"""
    ...
    print(f"Names and roles response: {response.content}")
    try:
...
def main():
    ...
    # Load the transcription data
    data = load_transcription_data(json_file_path)
